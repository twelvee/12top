<div class="panel panel-default">
	...
	<div class="row box-section">	
		<div class="col-sm-12">
				...
				<form method="POST" id="addwidget" class="form-horizontal">
		                ...
		                <div class="tab-pane active" id="tabhome">
							<div class="panel-body">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="number" class="form-control width-200 position-left" name="count" min="1" required>
									</div>	
								</div>
							</div>
							<div class="panel-footer">
								<button type="submit" name="addwidget" class="btn bg-teal btn-sm btn-raised position-left"><i class="fa fa-plus position-left"></i>Добавить виджет</button>
							</div>
						</div>
					</div>
				</form>
				...
				<table class="table table-hover">
					...
					<tbody>
						...
						<tr>
							<td>{$tv[name]}</td>
							<td>{$tv[type]}</td>
							<td>{if $tv[sortby]=="valdesc"} По убыванию {else} По возрастанию {/if}</td>
							<td>{$tv[count]}</td>
							<td>{$tv[tag]}</td>
